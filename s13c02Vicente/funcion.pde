void funcion(float x1, float y1, String vientoDirT, float vientoDir, float temMax, float temMax2, float temMin, float temMin2, float temMed, float temMed2, String event, float velvien, String temMednum) {
  ...
  ////temperatura media
  float maptemMed = map(temMed, -13, 28, 5, 40);
  float maptemMed2 = map(temMed2, -13, 28, 50, 150);
  fill(232, 182, 71, maptemMed2);
  noStroke();
  ellipse(x1, y1, maptemMed, maptemMed);



  ////temperatura maxima
  float maptemMax = map(temMax, -10, 34, 5, 40);
  float maptemMax2 = map(temMax2, -10, 34, 50, 150);
  fill(231, 76, 60, maptemMax2);
  noStroke();
  ellipse(x1, y1, maptemMax, maptemMax);

  ////temperatura minima
  float maptemMin = map(temMin, -17, 23, 40, 5);
  float maptemMin2 = map(temMin2, -17, 23, 50, 150);
